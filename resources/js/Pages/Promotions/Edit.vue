...
const imagePreview = ref(props.promotion.image_url ?? null)
const showCatalogPicker = ref(false)
const linkedTemplateIds = ref(
    (props.promotion.modifier_group_templates || []).map(t => t.id)
)

const form = useForm({
    name: props.promotion.name,
    description: props.promotion.description ?? '',
    price: props.promotion.price,
    production_cost: props.promotion.production_cost ?? '',
    image: null,
    is_active: props.promotion.is_active,
    active_days: props.promotion.active_days ? props.promotion.active_days.map(Number) : [],
    starts_at: props.promotion.starts_at ?? '',
    ends_at: props.promotion.ends_at ?? '',
    modifier_groups: (props.promotion.modifier_groups || []).map(g => ({
        id: g.id,
        name: g.name,
        ...
        options: g.options.map(o => ({
            ...
            production_cost: Number(o.production_cost),
...
function submit() {
    form.transform((data) => ({ ...data, _method: 'put' })).post(route('promotions.update', props.promotion.id), {
        forceFormData: true,
    })
}
</script>
...
<template>
    <Head title="Editar Promocion" />
    <AppLayout title="Editar Promocion">

        <h1 class="text-2xl font-bold text-gray-900 mb-6">Editar Promocion</h1>
        ...
        <div class="grid grid-cols-3 gap-6 pb-24">
            ...
            <div class="col-span-2 space-y-5">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 p-6">
                    ...
                    <p v-if="form.modifier_groups.length === 0 && linkedTemplates.length === 0" class="text-sm text-gray-400">
