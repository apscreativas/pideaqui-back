...
const form = useForm({
    ...
    min_purchase: parseFloat(props.coupon.min_purchase) > 0 ? props.coupon.min_purchase : '',
    starts_at: formatDatetime(props.coupon.starts_at),
    ends_at: formatDatetime(props.coupon.ends_at),
    max_uses_per_customer: props.coupon.max_uses_per_customer ?? '',
    max_total_uses: props.coupon.max_total_uses ?? '',
...
function submit() {
    form.put(route('coupons.update', props.coupon.id))
}
</script>

<template>
    <AppLayout title="Editar cupón">
        <div class="max-w-2xl">
            <!-- Header -->
            <div class="flex items-center gap-3 mb-6">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Editar cupón</h1>
                    <p class="text-sm text-gray-500 mt-0.5">
                        <span class="font-mono font-semibold">{{ coupon.code }}</span>
                        <span v-if="coupon.uses_count > 0" class="ml-2 text-gray-400">· {{ coupon.uses_count }} uso{{ coupon.uses_count !== 1 ? 's' : '' }}</span>
                    </p>
                </div>
            </div>
                ...
                <div class="bg-white rounded-xl border border-gray-100 p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            @input="onCodeInput"
                            maxlength="20"
                            class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm font-mono uppercase focus:outline-none focus:ring-2 focus:ring-[#FF5722]/20 focus:border-[#FF5722]"
                        />
